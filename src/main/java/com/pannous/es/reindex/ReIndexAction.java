...
public class ReIndexAction extends BaseRestHandler {
	...
	@Inject
	public ReIndexAction(Settings settings, Client client,
	...
	public void handleRequest(RestRequest request, RestChannel channel,
			String newTypeOverride, boolean internalCall) {
		...
		try {
			...
			String newType = newTypeOverride != null ? newTypeOverride
					: request.param("type");
			String searchType = newTypeOverride != null ? newTypeOverride
					: request.param("searchType");
			if (searchType == null || searchType.isEmpty())
				searchType = newType;

			int searchPort = request.paramAsInt("searchPort", 9200);
			String searchHost = request.param("searchHost", "localhost");
			boolean localAction = "localhost".equals(searchHost)
					&& searchPort == 9200;
			...
			if (localAction) {
				SearchRequestBuilder srb = createScrollSearch(searchIndexName,
						searchType, filter, hitsPerPage, withVersion,
						keepTimeInMinutes);
				SearchResponse sr = srb.execute().actionGet();
				rsp = new MySearchResponseES(client, sr, keepTimeInMinutes);
			} else {
				// TODO make it possible to restrict to a cluster
				rsp = new MySearchResponseJson(searchHost, searchPort,
						searchIndexName, searchType, filter,
						basicAuthCredentials, hitsPerPage, withVersion,
						keepTimeInMinutes);
			}

			// TODO make async and allow control of process from external (e.g.
			// stopping etc)
			// or just move stuff into a river?
			reindex(rsp, newIndexName, newType, withVersion, waitInSeconds);

			// TODO reindex again all new items => therefor we need a timestamp
			// field to filter
			// + how to combine with existing filter?

			logger.info("Finished reindexing of index " + searchIndexName
					+ " into " + newIndexName + ", query " + filter);

			if (!internalCall)
				channel.sendResponse(new XContentRestResponse(request, OK,
						builder));
	...
	public SearchRequestBuilder createScrollSearch(String oldIndexName,
			String oldType, String filter, int hitsPerPage,
			boolean withVersion, int keepTimeInMinutes) {
		SearchRequestBuilder srb = client.prepareSearch(oldIndexName)
				.setTypes(oldType).setVersion(withVersion).setSize(hitsPerPage)
	...
	public int reindex(MySearchResponse rsp, String newIndex, String newType,
			boolean withVersion, float waitSeconds) {
		...
		while (true) {
			...
			if (flushEnabled)
				client.admin().indices().flush(new FlushRequest(newIndex))
						.actionGet();
			...
			logger.debug("Progress " + collectedResults + "/" + total
					+ ". Time of update:"
					+ updateWatch.totalTime().getSeconds() + " query:"
		...
		for (MySearchHit hit : objects.getHits()) {
			...
			try {
				IndexRequest indexReq = Requests.indexRequest(indexName)
						.type(newType).id(hit.id()).source(hit.source());
				if (withVersion)
					indexReq.version(hit.version());

				brb.add(indexReq);
			} catch (Exception ex) {
				logger.warn("Cannot add object:" + hit
						+ " to bulkIndexing action." + ex.getMessage());
			}
		}
